...
| `src/backend/chroma.ts` | ChromaVectorStore 实现（HTTP 服务后端） | 否 |
...
| 后端 | 适用场景 | 部署方式 | 特点 |
|---|---|---|---|
| **Chroma** | 多用户、分布式、生产环境 | HTTP 服务（独立进程） | 内置向量索引、REST API、支持并发 |
| **SQLite** | 本地开发、单用户、离线、零外部依赖 | 嵌入式（同一进程） | 零安装、单文件数据库、WAL 模式支持并发读 |
